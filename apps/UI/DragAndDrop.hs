...
import           UI.Element                  (Dom, elClass', elDynAttr',
                                              offsetPosition)
import           UI.Event                    (EventResult, addEventListener,
                                              client, movement, offset)
...
draggable container tag attributes body = do
  rec (element, result) <- elDynAttr' tag attributes' body
      ...
      let mousedown = Dom.domEvent Dom.Mousedown element
      ...
      elementStart <- holdDyn (Point 0 0) $ traceEvent "elementPosition" $
        elementPosition <$> mousedown
